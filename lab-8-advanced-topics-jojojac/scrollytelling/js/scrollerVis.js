class ScrollerVis {
  ...
  initVis() {
    ...
    // We will edit attributes of the rectangles later – I just want you to know where we are drawing them.
  ...
  step3() {
    ...
    vis.rect.transition()
        .attr('opacity', 1)
        .attr('fill', d => d.trail==vis.dataLongestAssignment.trail ? vis.colorScale('highlighted') : vis.colorScale('inactive'))
        .attr('width', d => vis.config.cellWidth)
        .attr('height', d => vis.config.cellHeight)
        .attr('x', (d, i) => i % vis.config.columns * (vis.config.cellWidth + vis.config.cellSpacing))
        .attr('y', (d, i) => Math.floor(i / vis.config.columns) % vis.config.rows * (vis.config.cellHeight + vis.config.cellSpacing));
  ...
  step4() {
    ...
    vis.textG = vis.chart.selectAll('g') // 'g.bar-label')
